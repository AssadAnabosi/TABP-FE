...
export function HotelFormSheet({ hotel, open, onOpenChange }: HotelFormSheetProps) {
  ...
  const form = useForm<Input, unknown, Values>({
    // An admin must always pick an owner who holds the HotelOwner role.
    resolver: zodResolver(
      schema.refine((v) => v.ownerId !== '', { path: ['ownerId'], message: 'Pick an owner.' }),
    ),
    values: {
      name: hotel?.name ?? '',
  ...
  const save = useMutation({
    mutationFn: async ({ ownerId, ...body }: Values) => {
      if (!hotel) return hotelsApi.create({ ...body, ownerId })
    ...
    onSuccess: (saved) => {
      ...
      toast.success(hotel ? `${saved.name} updated.` : `${saved.name} created and approved.`)
      onOpenChange(false)
    },
    onError: (error) => {
  ...
  return (
    ...
    >
      <SheetContent className="w-full overflow-y-auto sm:max-w-xl">
        <SheetHeader>
          <SheetTitle>{hotel ? `Edit ${hotel.name}` : 'New hotel'}</SheetTitle>
          <SheetDescription>
            {hotel
        ...
        <SheetFooter className="flex-row justify-end">
          ...
          <Button type="submit" form="hotel-form" disabled={save.isPending}>
            {save.isPending ? 'Saving…' : hotel ? 'Save changes' : 'Create hotel'}
          </Button>
        </SheetFooter>
      </SheetContent>
    </Sheet>
